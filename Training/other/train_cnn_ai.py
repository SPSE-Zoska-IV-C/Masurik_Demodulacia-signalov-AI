...
class ComplexRadioDataset(Dataset):
    def __init__(self, data_dir, dtype=np.complex64, max_samples=9000):
...
class ComplexCNN1D(nn.Module):
    def __init__(self, max_samples):
        ...
        final_length = max_samples // 8  # 3x MaxPool1d(2)

        self.fc = nn.Sequential(
            nn.Linear(128 * final_length, 512),
            nn.ReLU(),
            nn.Linear(512, 32),
            nn.Sigmoid(),
        )
    ...
    def forward(self, x):
        x = x.view(x.size(0), 2, self.max_samples)
        x = self.cnn(x)
        x = x.flatten(1)
        return self.fc(x)
...
def train_model(
    ...
    max_samples = train_dataset.max_samples
    ...
    # --- Model ---
    model = ComplexCNN1D(max_samples=max_samples)
    ...
    torch.save(model.state_dict(), "CNN_radio_model_32bit.pth")
    print("Model saved to CNN_radio_model_32bit.pth")
